"""DRAGON_SCALE Real-time Anomaly Detection Flink Job
...
Outputs detected anomalies to dragon-scale-anomalies Kafka topic for
...
class AnomalyDetectionJob:
    ...
    Consumes from:
    - dragon-scale-network-events (XDP collector, data-collector)
    - dragon-scale-host-events (HIDS agent, hardening service)

    Produces to:
    - dragon-scale-anomalies (AI engine, alert service)
    """
    ...
    def _create_kafka_sources(self):
        ...
        CREATE TABLE network_events (
            ...
            'properties.group.id' = 'dragon-scale-anomaly-detection',
    ...
    def run(self):
        logger.info("Starting DRAGON_SCALE Anomaly Detection Job")
