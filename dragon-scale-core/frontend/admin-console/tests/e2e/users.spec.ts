...
test.describe('Users — management workflow', () => {
  ...
  test('displays all seeded users in the table', async ({ page }) => {
    ...
    await expect(tbody.getByText('demo@dragon-scale.local')).toBeVisible()
    await expect(tbody.getByText('analyst@dragon-scale.local')).toBeVisible()
  })
  ...
  test('current user row has no edit button', async ({ page }) => {
    ...
    const demoRow = page.locator('table tbody tr', { hasText: 'demo@dragon-scale.local' })
  ...
  test('opens inline edit for another user', async ({ page }) => {
    ...
    const analystRow = page.locator('table tbody tr', { hasText: 'analyst@dragon-scale.local' })
