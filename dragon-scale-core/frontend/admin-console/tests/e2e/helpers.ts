...
const USERS_SEED = [
  { ...demoUser },
  { id: 2, username: 'analyst', email: 'analyst@dragon-scale.local', role: 'security_analyst', status: 'active', created_at: '2026-02-01T00:00:00Z', last_login: '2026-03-12T10:00:00Z' },
  { id: 3, username: 'viewer1', email: 'viewer1@dragon-scale.local', role: 'viewer', status: 'active', created_at: '2026-02-15T00:00:00Z', last_login: null },
]
...
export const seedAuthState = async (page: Page) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem('dragon-scale-auth', JSON.stringify(state))
    localStorage.setItem('dragon-scale-token', 'test-access-token')
    localStorage.setItem('dragon-scale-refresh-token', 'test-refresh-token')
  })
}
