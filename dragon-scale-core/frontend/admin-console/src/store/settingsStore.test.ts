...
const defaults: PlatformSettings = {
  organizationName: "DRAGON_SCALE Security",
  timezone: "UTC",
...
describe("settingsStore", () => {
  ...
  describe("initial state", () => {
    it("has correct default organization name", () => {
      expect(useSettingsStore.getState().organizationName).toBe(
        "DRAGON_SCALE Security",
      );
    });
  ...
  describe("persistence", () => {
    it("store is created with persist middleware (name: dragon-scale-settings)", () => {
      const store = useSettingsStore as unknown as {
        persist: { getOptions: () => { name: string } };
      };
      expect(store.persist.getOptions().name).toBe("dragon-scale-settings");
    });
  });
