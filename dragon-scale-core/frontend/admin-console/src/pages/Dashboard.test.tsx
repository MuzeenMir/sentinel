...
vi.mock("recharts", () => {
  const FakeChart = ({ children }: { children?: React.ReactNode }) => (
...
describe("Dashboard", () => {
  ...
  it("shows fallback warning banner when API returns errors", async () => {
    ...
    const banner = await screen.findByText(
      /live telemetry is unavailable/i,
      {},
      { timeout: 3000 },
    );
  ...
  it('shows "No recent threats detected." when recentThreats is empty', async () => {
    ...
    expect(
      await screen.findByText(/no recent threats detected/i),
    ).toBeInTheDocument();
  });
  ...
  it("opens SSE stream on mount and closes on unmount", async () => {
    ...
    const closeFn = vi.fn();
    vi.mocked(createSseClient).mockReturnValue({
      close: closeFn,
