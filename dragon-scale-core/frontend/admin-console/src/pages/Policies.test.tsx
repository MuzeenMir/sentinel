...
describe("Policies", () => {
  ...
  it("shows empty state when no policies exist", async () => {
    ...
    expect(
      await screen.findByText(/no policies available/i),
    ).toBeInTheDocument();
  });
  ...
  it('opens create modal when "+ Create Policy" is clicked', async () => {
    ...
    expect(
      screen.getByText("Create Policy", { selector: "h3" }),
    ).toBeInTheDocument();
  ...
  it("closes create modal when Cancel is clicked", async () => {
    ...
    expect(
      screen.queryByText("Create Policy", { selector: "h3" }),
  ...
  it("shows validation message when name is empty on submit", async () => {
    ...
    expect(
      await screen.findByText(/policy name is required/i),
    ).toBeInTheDocument();
  ...
  it("submits create policy form with valid data", async () => {
    ...
    const modal = screen
      .getByText("Create Policy", { selector: "h3" })
      .closest("div.card")!;
  ...
  it("shows success message after successful creation", async () => {
    ...
    expect(
      await screen.findByText(/policy created successfully/i),
    ).toBeInTheDocument();
  });
