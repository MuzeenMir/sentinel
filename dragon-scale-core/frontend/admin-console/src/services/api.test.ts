...
describe("API service", () => {
  ...
  beforeEach(async () => {
    ...
    mockState = (authMock as unknown as { __mockState: typeof mockState })
      .__mockState;
  ...
  describe("response interceptor (401 refresh)", () => {
    it("attempts token refresh on 401", async () => {
      ...
      mockState.refreshAccessToken.mockRejectedValue(
  ...
  describe("policyApi", () => {
    ...
    it("updatePolicy sends PUT to /api/v1/policies/:id", () => {
      ...
      expect(putSpy).toHaveBeenCalledWith("/api/v1/policies/pol-1", {
        name: "Updated",
      });
      putSpy.mockRestore();
    });
  ...
  describe("alertApi", () => {
    it("getAlerts sends GET to /api/v1/alerts with params", () => {
      ...
      expect(getSpy).toHaveBeenCalledWith("/api/v1/alerts", {
        params: { status: "new", severity: "high", page: 2 },
      });
      getSpy.mockRestore();
    });
    ...
    it("ignore sends PUT to /api/v1/alerts/:id with status=ignored", () => {
      ...
      expect(putSpy).toHaveBeenCalledWith("/api/v1/alerts/a-1", {
        status: "ignored",
      });
      putSpy.mockRestore();
    });
  ...
  describe("complianceApi", () => {
    ...
    it("runAssessment sends POST to /api/v1/assess", () => {
      ...
      expect(postSpy).toHaveBeenCalledWith("/api/v1/assess", {
        framework: "GDPR",
        policies: [],
      });
      postSpy.mockRestore();
