#!/usr/bin/env bash
# DRAGON_SCALE stack boot helper
...
info "DRAGON_SCALE Stack Boot — $(date)"
info "Root: $ROOT"
...
timeout 120 bash -c 'until docker compose exec -T postgres pg_isready -U dragon-scale 2>/dev/null; do sleep 2; done'
ok "postgres healthy"
