# DRAGON_SCALE – Troubleshooting

## Postgres container exits with code 3

**Symptom:** `✘ Container dragon-scale-postgres Error` and `dependency failed to start: container dragon-scale-postgres exited (3)`.
...
```bash
cd /mnt/c/Users/MirMu/Projects/dragon-scale/dragon-scale-core
docker compose logs postgres
```

(or `docker logs dragon-scale-postgres`). The last lines usually show the real error (init script, permissions, or port).
...
```bash
docker compose --env-file dragon_scale_env down -v
docker compose --env-file dragon_scale_env up -d
```
...
Once `dragon-scale-postgres` stays up and healthy:

- Auth and API gateway depend on it and should start.
- Use: `docker compose --env-file dragon_scale_env up -d` and check with `docker compose ps`.
