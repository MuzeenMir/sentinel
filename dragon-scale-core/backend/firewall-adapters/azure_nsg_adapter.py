...
class AzureNSGAdapter(FirewallAdapter):
    ...
    def __init__(
        ...
        self._priority_counter = 1000  # Starting priority for DRAGON_SCALE rules
    ...
    def add_rule(self, rule: FirewallRule) -> Dict[str, Any]:
        ...
        try:
            ...
            operation = self._client.security_rules.begin_create_or_update(
                self.resource_group, self.nsg_name, f"dragon-scale-{rule.id}", azure_rule
            )
            ...
            return {"success": True, "rule_id": rule.id, "nsg_name": self.nsg_name}
    ...
    def remove_rule(self, rule_id: str) -> Dict[str, Any]:
        ...
        try:
            operation = self._client.security_rules.begin_delete(
                self.resource_group, self.nsg_name, f"dragon-scale-{rule_id}"
            )

            operation.result()  # Wait for completion

            del self._rules_cache[rule_id]
            logger.info(f"Removed Azure NSG rule: dragon-scale-{rule_id}")
    ...
    def _build_azure_rule(self, rule: FirewallRule) -> Dict[str, Any]:
        ...
        params = {
            ...
            "direction": direction,
            "description": f"DRAGON_SCALE: {rule.description}",
        }
