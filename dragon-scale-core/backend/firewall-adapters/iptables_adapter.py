...
class IptablesAdapter(FirewallAdapter):
    ...
    Uses a dedicated chain (DRAGON_SCALE) for managed rules.
    """

    CHAIN_NAME = "DRAGON_SCALE"
    ...
    def is_available(self) -> bool:
        ...
        try:
            result = subprocess.run(["iptables", "--version"], capture_output=True)
            return result.returncode == 0
        except FileNotFoundError:
            return False

    def _ensure_chain(self):
        """Ensure DRAGON_SCALE chain exists."""
        if not self.is_available:
            return

        # Check if chain exists
        result = subprocess.run(
            ["iptables", "-L", self.CHAIN_NAME, "-n"], capture_output=True
        )
        ...
        if result.returncode != 0:
            ...
            # Add jump to DRAGON_SCALE chain from INPUT
            subprocess.run(
                ["iptables", "-I", "INPUT", "-j", self.CHAIN_NAME], capture_output=True
            )

            # Add jump from OUTPUT for egress rules
            subprocess.run(
                ["iptables", "-I", "OUTPUT", "-j", self.CHAIN_NAME], capture_output=True
            )

            logger.info("Created DRAGON_SCALE iptables chain")
    ...
    def add_rule(self, rule: FirewallRule) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "rule_id": rule.id, "command": " ".join(cmd)}
            else:
    ...
    def clear_dragon_scale_rules(self) -> Dict[str, Any]:
        """Clear all DRAGON_SCALE rules."""
        if not self.is_available:
            return {"success": False, "error": "iptables not available"}

        try:
            # Flush DRAGON_SCALE chain
            subprocess.run(["iptables", "-F", self.CHAIN_NAME], capture_output=True)

            count = len(self._rules_cache)
            self._rules_cache.clear()

            logger.info(f"Cleared {count} DRAGON_SCALE rules")
    ...
    def _build_add_command(self, rule: FirewallRule) -> List[str]:
        ...
        if rule.action == FirewallAction.RATE_LIMIT and rule.rate_limit:
            cmd.extend(
                [
                    ...
                    "limit",
                    "--limit",
                    f"{rule.rate_limit}/sec",
                    "--limit-burst",
                    str(rule.rate_limit_burst or rule.rate_limit),
                ]
            )
        ...
        cmd.extend(["-m", "comment", "--comment", f"DRAGON_SCALE:{rule.id}"])
