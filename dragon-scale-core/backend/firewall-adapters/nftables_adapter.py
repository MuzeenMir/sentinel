...
class NftablesAdapter(FirewallAdapter):
    ...
    Uses a dedicated table and chain for DRAGON_SCALE rules.
    """

    TABLE_NAME = "dragon-scale"
    INPUT_CHAIN = "dragon_scale_input"
    OUTPUT_CHAIN = "dragon_scale_output"
    ...
    def is_available(self) -> bool:
        ...
        try:
            result = subprocess.run(["nft", "--version"], capture_output=True)
            return result.returncode == 0
        except FileNotFoundError:
            return False

    def _ensure_table(self):
        """Ensure DRAGON_SCALE table and chains exist."""
...
"""
        ...
        try:
            ...
            result = subprocess.run(
                ["nft", "list", "table", "inet", self.TABLE_NAME], capture_output=True
            )

            if result.returncode != 0:
                # Create table and chains
                process = subprocess.run(
                    ["nft", "-f", "-"], input=nft_script, capture_output=True, text=True
                )

                if process.returncode == 0:
                    logger.info("Created DRAGON_SCALE nftables table")
                else:
    ...
    def add_rule(self, rule: FirewallRule) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "rule_id": rule.id, "nft_rule": nft_rule}
            else:
    ...
    def _build_nft_rule(self, rule: FirewallRule) -> str:
        ...
        if rule.action == FirewallAction.RATE_LIMIT and rule.rate_limit:
            parts.append(
                f"limit rate {rule.rate_limit}/second burst {rule.rate_limit_burst or rule.rate_limit} packets"
            )
        ...
        action_map = {
            ...
            FirewallAction.LOG: 'log prefix "DRAGON_SCALE: "',
            FirewallAction.RATE_LIMIT: "accept",
        }
        parts.append(action_map.get(rule.action, "drop"))

        # Comment
        parts.append(f'comment "DRAGON_SCALE:{rule.id}"')
