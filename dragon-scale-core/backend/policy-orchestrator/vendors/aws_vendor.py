...
class AWSSecurityGroupVendor(BaseVendor):
        ...
        try:
            ...
            if ingress_rules:
                self.ec2_client.authorize_security_group_ingress(
                    GroupId=self.security_group_id, IpPermissions=ingress_rules
                )
                applied += len(ingress_rules)

            if egress_rules:
                self.ec2_client.authorize_security_group_egress(
                    GroupId=self.security_group_id, IpPermissions=egress_rules
                )
    ...
    def _translate_rule(self, rule: Dict) -> Dict[str, Any]:
        ...
        aws_rule["IpRanges"] = [
            {"CidrIp": source, "Description": f"DRAGON_SCALE: {rule.get('id', 'unknown')}"}
        ]
