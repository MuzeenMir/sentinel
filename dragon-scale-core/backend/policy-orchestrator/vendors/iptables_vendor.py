...
class IptablesVendor(BaseVendor):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.chain = config.get("chain", "DRAGON_SCALE") if config else "DRAGON_SCALE"
    ...
    def connect(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["iptables", "--version"], capture_output=True, text=True
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"vendor": self.vendor_name, "connected": False, "error": str(e)}

    def _ensure_chain_exists(self):
        """Ensure the DRAGON_SCALE chain exists."""
        try:
            # Check if chain exists
            result = subprocess.run(
                ["iptables", "-t", self.table, "-L", self.chain], capture_output=True
            )
    ...
    def _build_iptables_command(self, rule: Dict) -> str:
        ...
        # Add comment
        if rule.get("id"):
            parts.extend(["-m", "comment", "--comment", f"DRAGON_SCALE:{rule['id']}"])
