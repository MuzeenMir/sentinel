...
_TABLE = "dragon_scale_fw"
_FAMILY = "inet"

_CHAIN_MAP = {
    "INBOUND": "dragon_scale_input",
    "OUTBOUND": "dragon_scale_output",
}
_HOOK_MAP = {
    "dragon_scale_input": "input",
    "dragon_scale_output": "output",
}
_ACTION_VERDICT = {
    "ALLOW": "accept",
    "DENY": "drop",
...
class NftablesAdapter(BaseVendorAdapter):
    """Translates DRAGON_SCALE rules to nftables ruleset format."""
    ...
    def translate_rules(self, rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        translated = []
        for rule in rules:
            chain = _CHAIN_MAP.get(rule.get("direction", "INBOUND"), "dragon_scale_input")
    ...
    def _rule_to_statement(rule: Dict[str, Any]) -> str:
        ...
        if action_key == "RATE_LIMIT":
            ...
            prefix = f"dragon-scale:{rule.get('id', 'rule')}:"
        ...
        rule_id = rule.get("id", "")
        if rule_id:
            parts.append(f'comment "dragon-scale:{rule_id}"')
    ...
    def _remove_by_handle(rule_ids: Set[str]) -> Dict[str, Any]:
        """Locate rules by their dragon-scale comment tag and delete by handle."""
        try:
            proc = subprocess.run(
            ...
            for item in data.get("nftables", []):
                ...
                for rid in rule_ids:
                    if f"dragon-scale:{rid}" in comment:
