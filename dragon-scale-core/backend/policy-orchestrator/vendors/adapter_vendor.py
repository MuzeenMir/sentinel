...
def _policy_rule_to_firewall_rule(rule: Dict[str, Any]) -> Optional[FirewallRule]:
    ...
    if action == FirewallAction.RATE_LIMIT and isinstance(rule.get("rate_limit"), dict):
        rate_limit = rule["rate_limit"].get("packets_per_second") or rule[
            "rate_limit"
        ].get("pps")
    ...
    fw_rule = FirewallRule(
        ...
        direction="ingress"
        if (rule.get("direction") or "inbound") == "inbound"
        else "egress",
        source_ip=source_ip,
...
class AdapterVendor(BaseVendor):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self._connected = (
                self._adapter.is_available
                if hasattr(self._adapter, "is_available")
                else True
            )
    ...
    def apply_rules(self, rules: List[Dict]) -> Dict[str, Any]:
        if not self._adapter or not HAS_ADAPTERS:
            return {
                "success": False,
                "message": "Adapter not connected or package missing",
            }
        applied = 0
        errors = []
        for rule in rules:
        ...
        return {
            ...
            "message": f"Applied {applied}/{len(rules)} rules"
    ...
    def remove_rules(self, rules: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "success": len(errors) == 0,
            "message": f"Removed {removed}/{len(rules)} rules"
            + ("; " + "; ".join(errors) if errors else ""),
            "removed": removed,
            "total": len(rules),
            "errors": errors,
    ...
    def translate_rules(self, rules: List[Dict]) -> List[str]:
        ...
        for rule in rules:
            ...
            if fw_rule:
                translated.append(
                    f"{fw_rule.action.value} {fw_rule.source_ip} -> {fw_rule.destination_port or '*'}"
                )
        return translated
