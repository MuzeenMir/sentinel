"""Integration dispatcher -- routes DRAGON_SCALE events to configured external systems.

Supports:
- Webhooks (generic HTTP POST)
- SIEM: Splunk HEC, Elastic SIEM, Microsoft Dragon Scale (CEF/LEEF format)
- SOAR: Palo Alto XSOAR, Tines (webhook trigger)
- Ticketing: ServiceNow, Jira
- Notifications: Slack, Email (via alert-service)

Each integration type is a thin adapter that transforms a DRAGON_SCALE event
...
class WebhookAdapter(IntegrationAdapter):
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        if secret:
            ...
            headers["X-Dragon Scale-Signature"] = sig
...
class SplunkHECAdapter(IntegrationAdapter):
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        index = self.config.get("index", "dragon-scale")
        source_type = self.config.get("sourcetype", "dragon-scale:event")
...
class ElasticSIEMAdapter(IntegrationAdapter):
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        index = self.config.get("index", "dragon-scale-events")
        ...
        doc = {
            ...
            "event": {"kind": "alert", "module": "dragon-scale"},
            **event,
        }
...
class XSOARAdapter(IntegrationAdapter):
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        incident = {
            "name": f"DRAGON_SCALE: {event.get('type', 'security_event')}",
            "type": "DRAGON_SCALE Alert",
...
class ServiceNowAdapter(IntegrationAdapter):
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        incident = {
            "short_description": f"DRAGON_SCALE: {event.get('type', 'security_event')}",
    ...
    def send(self, event: Dict[str, Any]) -> bool:
        ...
        issue = {
            "fields": {
                ...
                "summary": f"DRAGON_SCALE: {event.get('type', 'security_event')}",
                "description": json.dumps(event, indent=2),
                "issuetype": {"name": "Bug"},
                "priority": {
...
def format_cef(event: Dict[str, Any]) -> str:
    """Format a DRAGON_SCALE event as CEF (Common Event Format) for SIEM ingestion."""
    ...
    return (
        f"CEF:0|DRAGON_SCALE|SecurityPlatform|1.0|"
...
def format_leef(event: Dict[str, Any]) -> str:
    """Format a DRAGON_SCALE event as LEEF 2.0 (Log Event Extended Format) for IBM QRadar."""
    severity_map = {"low": 3, "medium": 5, "high": 7, "critical": 10}
    sev = severity_map.get(event.get("severity", "medium"), 5)
    ...
    return (
        f"LEEF:2.0|DRAGON_SCALE|SecurityPlatform|1.0|"
        f"{event.get('type', 'generic')}|"
        f"{attr_str}"
    )
