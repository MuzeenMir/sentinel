"""
DRAGON_SCALE Enhanced Data Collector
...
app = Flask(__name__)
configure_logging(service_name="data-collector")
init_metrics(app, service_name="data-collector")
...
class CIMNormalizer:
    ...
    def normalize(
        self, data: Dict[str, Any], source_type: DataSourceType
    ) -> Dict[str, Any]:
        """
        ...
        normalized = {
            ...
            "src_port": self._normalize_port(
                data.get("source_port") or data.get("src_port")
            ),
            "dest_port": self._normalize_port(
                data.get("dest_port") or data.get("destination_port")
            ),
...
class NetFlowParser:
    ...
    def _parse_v5(self, data: bytes, source_addr: Tuple[str, int]) -> List[Dict]:
        ...
        for i in range(count):
            ...
            # Calculate duration
            flow_record["duration"] = (
                flow_record["end_time"] - flow_record["start_time"]
            )

            # Normalize and add
            normalized = self.normalizer.normalize(
                flow_record, DataSourceType.NETFLOW_V5
            )
...
class SFlowParser:
    ...
    def parse(self, data: bytes, source_addr: Tuple[str, int]) -> List[Dict]:
        ...
        try:
            ...
            for i in range(min(num_samples, 100)):  # Limit samples
                ...
                sample_type, sample_len = struct.unpack(
                    "!II", data[offset : offset + 8]
                )
                offset += 8

                if sample_type == 1:  # Flow sample
                    sample = self._parse_flow_sample(
                        data[offset : offset + sample_len], source_addr
                    )
                    if sample:
                        normalized = self.normalizer.normalize(
                            sample, DataSourceType.SFLOW
                        )
                        records.append(normalized)
    ...
    def _parse_flow_sample(
        self, data: bytes, source_addr: Tuple[str, int]
    ) -> Optional[Dict]:
...
class NetworkTrafficCollector:
    ...
    def start_netflow_listener(self):
        ...
        try:
            ...
            logger.info(
                f"Started NetFlow listener on port {app.config['NETFLOW_PORT']}"
            )
        ...
        try:
            ...
            if eth_protocol == 8:  # IPv4
                ...
                if protocol == TCP_PROTOCOL:
                    ...
                    raw_data.update({"icmp_type": icmph[0], "icmp_code": icmph[1]})
...
def start_background_collection():
    ...
    netflow_thread = threading.Thread(
        target=collector.start_netflow_listener, daemon=True
    )
...
def get_traffic():
    ...
    try:
        stats = {"sources": {}, "destinations": {}, "protocols": {}, "directions": {}}
...
def ingest_data():
    ...
    try:
        ...
        return jsonify(
            {"message": "Data ingested successfully", "processed": processed}
        ), 200
...
def get_threats():
    ...
    try:
        ...
        return jsonify({"threats": threats, "total": len(threats)}), 200
