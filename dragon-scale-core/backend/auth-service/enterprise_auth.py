...
def _map_role_from_groups(groups: list) -> str:
    """Map IdP groups to Dragon Scale role via SSO_GROUP_ROLE_MAP."""
    for group in groups:
...
def _issue_dragon_scale_tokens(user):
    """Issue Dragon Scale JWT access + refresh tokens for a user."""
    additional_claims = {
...
def oidc_callback():
    """Exchange the authorization code for tokens, JIT-provision user, issue Dragon Scale JWT."""
    meta = _get_oidc_metadata()
    if not meta:
        return jsonify({"error": "OIDC not configured"}), 501
    ...
    try:
        ...
        result = _issue_dragon_scale_tokens(user)
...
SAML_SP_ENTITY_ID = os.environ.get(
    "SAML_SP_ENTITY_ID", "https://dragon-scale.local/saml/metadata"
)
    ...
    return {
        ...
        "sp": {
            ...
            "assertionConsumerService": {
                "url": SAML_SP_ACS_URL
                or "https://dragon-scale.local/api/v1/auth/sso/saml/acs",
...
def saml_acs():
    """Assertion Consumer Service -- validate SAML response and issue Dragon Scale JWT."""
    if not SAML_IDP_METADATA_URL:
        return jsonify({"error": "SAML not configured"}), 501
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
  ...
  <md:SPSSODescriptor
    ...
    <md:AssertionConsumerService
        Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
        Location="{SAML_SP_ACS_URL or 'https://dragon-scale.local/api/v1/auth/sso/saml/acs'}"
        index="0"
        isDefault="true"/>
  </md:SPSSODescriptor>
</md:EntityDescriptor>"""
...
def mfa_enroll():
    ...
    provisioning_uri = totp.provisioning_uri(name=user.email, issuer_name="DRAGON_SCALE")
...
def scim_create_user():
    """SCIM 2.0 CreateUser endpoint — provisions user in Dragon Scale DB."""
    data = request.get_json() or {}
