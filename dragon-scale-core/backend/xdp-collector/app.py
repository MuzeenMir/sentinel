"""DRAGON_SCALE XDP Collector -- eBPF-powered network flow monitoring.
...
configure_logging(service_name="xdp-collector")
logger = logging.getLogger("dragon-scale.xdp-collector")


app = Flask(__name__)
CORS(app)
init_metrics(app, service_name="xdp-collector")
...
app.config["KAFKA_TOPIC"] = os.environ.get("KAFKA_TOPIC", "dragon-scale-network-events")
...
class XDPCollectorService:
    ...
    def _on_flow_event(self, event: Any) -> None:
        ...
        XDP_PACKETS.labels(action="pass").inc()
