"""DRAGON_SCALE HIDS Agent -- eBPF-powered host intrusion detection.
...
configure_logging(service_name="hids-agent")
logger = logging.getLogger("dragon-scale.hids-agent")


app = Flask(__name__)
CORS(app)
init_metrics(app, service_name="hids-agent")
...
KAFKA_TOPIC = os.environ.get("KAFKA_TOPIC", "dragon-scale-host-events")
...
class HIDSAgentService:
    ...
    def on_event(self, event: Any) -> None:
        ...
        if isinstance(event, ProcessExecEvent):
            ...
            EBPF_EVENTS.labels(event_type="priv_escalation").inc()
            ...
            EBPF_EVENTS.labels(event_type="module_load").inc()
