...
// DRAGON_SCALE privilege escalation and module load monitor.
...
int dragon_scale_setuid(struct trace_event_raw_sys_enter_setuid *ctx)
{
    ...
    evt->event_type = DRAGON_SCALE_EVENT_PRIV_ESCALATION;
...
int dragon_scale_module_load(struct trace_event_raw_module_load *ctx)
{
    ...
    evt->event_type = DRAGON_SCALE_EVENT_MODULE_LOAD;
    evt->timestamp_ns = bpf_ktime_get_ns();
    evt->pid = bpf_get_current_pid_tgid() >> 32;
    evt->uid = bpf_get_current_uid_gid() & 0xFFFFFFFF;
