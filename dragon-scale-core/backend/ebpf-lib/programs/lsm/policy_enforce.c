...
// DRAGON_SCALE LSM-based runtime policy enforcement.
                                         ...
                                         const char *detail, int detail_len)
{
    ...
    evt->event_type = DRAGON_SCALE_EVENT_POLICY_DECISION;
...
SEC("lsm/socket_bind")
int BPF_PROG(dragon_scale_socket_bind, struct socket *sock,
...
int BPF_PROG(dragon_scale_module_request, char *kmod_name, int ret)
{
    if (ret != 0)
        return ret;
