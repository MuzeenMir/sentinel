...
Validates that:
1. The iptables adapter creates the DRAGON_SCALE chain when iptables is available.
2. A DENY rule for a given source IP is correctly translated to an iptables
   DROP command.
3. The rule is visible in the DRAGON_SCALE chain.
...
class TestIptablesAdapterIntegration:
    ...
    def test_chain_exists_after_init(self, adapter):
        ...
        assert (
            result.returncode == 0
        ), f"DRAGON_SCALE chain not found:\n{result.stderr.decode()}"
