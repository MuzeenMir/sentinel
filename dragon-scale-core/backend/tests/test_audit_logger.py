"""
Tests for the DRAGON_SCALE SOC2 audit logger.
...
class TestAuditLog:
    ...
    def test_stores_in_category_sorted_set(self, fake_redis):
        ...
        assert "dragon-scale:audit:auth" in fake_redis.sorted_sets
        assert len(fake_redis.sorted_sets["dragon-scale:audit:auth"]) == 1
    ...
    def test_stores_in_global_index(self, fake_redis):
        ...
        assert "dragon-scale:audit:index" in fake_redis.sorted_sets
        assert len(fake_redis.sorted_sets["dragon-scale:audit:index"]) == 1
    ...
    def test_increments_stats(self, fake_redis):
        ...
        stats = fake_redis.hashes.get("dragon-scale:audit:stats", {})
    ...
    def test_record_contains_integrity_hash(self, fake_redis):
        ...
        raw = fake_redis.sorted_sets["dragon-scale:audit:auth"][0][1]
    ...
    def test_record_fields(self, fake_redis):
        ...
        raw = fake_redis.sorted_sets["dragon-scale:audit:config_change"][0][1]
        record = json.loads(raw)
    ...
    def test_sets_ttl_on_category_key(self, fake_redis):
        ...
        assert "dragon-scale:audit:alert" in fake_redis.expiry
        # Default 365 days
        assert fake_redis.expiry["dragon-scale:audit:alert"] == 365 * 86400

    def test_default_actor_is_system(self, fake_redis):
        with patch("audit_logger._in_request_context", return_value=False):
            audit_log(AuditCategory.SYSTEM, "boot", redis_client=fake_redis)
        raw = fake_redis.sorted_sets["dragon-scale:audit:system"][0][1]
        record = json.loads(raw)
