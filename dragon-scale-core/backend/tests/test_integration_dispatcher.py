"""
Tests for the DRAGON_SCALE Integration Dispatcher (SIEM, SOAR, Ticketing, Webhooks).
...
class TestWebhookAdapter:
    ...
    def test_hmac_signature_added_with_secret(self):
        ...
        assert "X-Dragon Scale-Signature" in headers
...
class TestElasticSIEMAdapter:
    def test_send_success(self):
        ...
        assert "dragon-scale-events/_doc" in str(url) or "dragon-scale-events/_doc" in str(
            mock_post.call_args
        )
...
class TestFormatCEF:
    def test_basic_format(self):
        ...
        assert cef.startswith("CEF:0|DRAGON_SCALE|SecurityPlatform|1.0|")
...
class TestFormatLEEF:
    def test_basic_format(self):
        ...
        assert leef.startswith("LEEF:2.0|DRAGON_SCALE|SecurityPlatform|1.0|")
