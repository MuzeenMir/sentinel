"""DRAGON_SCALE Hardening Service -- OS hardening + eBPF runtime enforcement.
...
logger = logging.getLogger("dragon-scale.hardening-service")


app = Flask(__name__)
CORS(app)
init_metrics(app, service_name="hardening-service")


HOST_ROOT = os.environ.get("HOST_ROOT", "/host")
BACKUP_DIR = os.environ.get("BACKUP_DIR", "/var/lib/dragon-scale/backups")
KAFKA_TOPIC = os.environ.get("KAFKA_TOPIC", "dragon-scale-hardening-events")
...
class HardeningService:
    ...
    def scan(self) -> List[CheckResult]:
        ...
        posture = self.stats.to_dict()["posture_score"]
        HARDENING_POSTURE.set(posture)
