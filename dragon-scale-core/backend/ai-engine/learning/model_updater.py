...
class ModelUpdater:
    ...
    def __init__(
        self, models_dir: str = "/models", check_interval: int = 60, redis_client=None
    ):
    ...
    def _subscribe_to_updates(self):
        ...
        def listener():
            try:
                ...
                pubsub.subscribe("dragon-scale:model_update")
                ...
                for message in pubsub.listen():
                    if message["type"] == "message":
                        ...
                        if model_name and model_name in self._loaders:
                            logger.info(
                                f"Received update notification for {model_name}"
                            )
                            self._load_model(model_name)
