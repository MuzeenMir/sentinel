#!/usr/bin/env python3
"""
DRAGON_SCALE Training Pipeline -- train_all.py
...
# PROJECT_ROOT must be inserted LAST (at index 0) so that dragon-scale-core/training/
...
logger = logging.getLogger("dragon-scale.training")
...
def train_drl(data: Dict, output_dir: str, device: str) -> Dict[str, float]:
    ...
    for episode in range(n_episodes):
        ...
        for _ in range(batch_size):
            ...
            log_probs.append(float(np.log(action_probs[action] + 1e-8)))
            values.append(value)
        ...
        # Compute discounted returns and advantages (GAE-lambda)
        returns = []
        gae = 0.0
        gamma, lam = 0.99, 0.95
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="DRAGON_SCALE Training Pipeline",
...
def main() -> None:
    ...
    args = parse_args()
    logger.info("DRAGON_SCALE Training Pipeline")
