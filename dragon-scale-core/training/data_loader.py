"""
Dataset loaders and preprocessors for DRAGON_SCALE training.
...
the DRAGON_SCALE ThreatCategory enum, engineers the 50-feature vector expected
...
# CIC-IDS2018 label → DRAGON_SCALE category
...
# CIC-IDS2017 label → DRAGON_SCALE category
...
# UNSW-NB15 attack_cat → DRAGON_SCALE category
...
def load_cicids2018(
    ...
    for fp in csv_files:
        ...
        if label_col is None:
            ...
            df["_dragon_scale_y"] = df["_dragon_scale_label"].map(LABEL_TO_IDX)
            drop_cols = [
                c
                for c in df.columns
                if c
        ...
        y_chunk = df["_dragon_scale_y"].values.astype(np.int64)
...
def load_cicids2017(
    ...
    y = df["_dragon_scale_y"].values.astype(np.int64)
...
def load_unsw_nb15(
    ...
    if label_col is None:
        if "label" in df.columns:
            df["_dragon_scale_label"] = df["label"].apply(
