#!/usr/bin/env bash
# =============================================================================
# DRAGON_SCALE Training Environment Setup
...
log()  { echo -e "${BLUE}[DRAGON_SCALE]${NC} $*"; }
ok()   { echo -e "${GREEN}[OK]${NC} $*"; }
warn() { echo -e "${YELLOW}[WARN]${NC} $*"; }
err()  { echo -e "${RED}[ERR]${NC} $*"; }

log "============================================="
log "  DRAGON_SCALE Local Training Environment Setup"
log "============================================="
...
if [[ ! -f "${ENV_FILE}" ]]; then
    ...
    cat > "${ENV_FILE}" <<EOF
# DRAGON_SCALE Training Environment Variables
DRAGON_SCALE_TRAINING_DEVICE=cuda
DRAGON_SCALE_DATA_PATH=${SCRIPT_DIR}/datasets/data
DRAGON_SCALE_OUTPUT_PATH=${REPO_ROOT}/backend/ai-engine/trained_models
DRAGON_SCALE_MAX_ROWS=
DRAGON_SCALE_DATASET=cicids2018,cicids2017,unsw_nb15

# RAM-friendly batch sizes for 14GB system
DRAGON_SCALE_BATCH_SIZE_AUTOENCODER=256
DRAGON_SCALE_BATCH_SIZE_LSTM=64
