...
###############################################################################
# DRAGON_SCALE Training - EC2 Instance Environment Setup
#
...
REPO_URL="${DRAGON_SCALE_REPO:-https://github.com/MuzeenMir/dragon-scale.git}"
BRANCH="${DRAGON_SCALE_BRANCH:-main}"
PYTHON_VERSION="3.12"
# Ubuntu 24.04 NVIDIA repo has 12-5/12-6, not 12-1; use 12-6 for compatibility
CUDA_VERSION="${CUDA_VERSION:-12-6}"
PROJECT_DIR="$HOME/dragon-scale"
VENV_DIR="$HOME/dragon-scale-venv"
...
# ── 3. Clone project ─────────────────────────────────────────────────────────
clone_project() {
    section "Cloning DRAGON_SCALE project"
...
setup_python_env() {
    ...
    if [ -f "${PROJECT_DIR}/dragon-scale-core/training/requirements.txt" ]; then
        pip install -r "${PROJECT_DIR}/dragon-scale-core/training/requirements.txt"
    else
        log "WARNING: training/requirements.txt not found, installing individual deps"
        pip install -r "${PROJECT_DIR}/dragon-scale-core/backend/ai-engine/requirements.txt"
        pip install -r "${PROJECT_DIR}/dragon-scale-core/backend/drl-engine/requirements.txt"
    fi
    ...
    if ! grep -q "dragon-scale-venv" "$HOME/.bashrc"; then
...
"
    ...
    log "      bash ${PROJECT_DIR}/dragon-scale-core/training/download_datasets.sh"
    log ""
    ...
    log "      cd ${PROJECT_DIR}/dragon-scale-core"
...
# ── Main ──────────────────────────────────────────────────────────────────────
main() {
    log "=== DRAGON_SCALE EC2 Environment Setup ==="
