...
###############################################################################
# DRAGON_SCALE Training - EC2 Instance Provisioning
#
...
KEY_NAME="${KEY_NAME:-dragon-scale-training}"
SG_NAME="${SG_NAME:-dragon-scale-training-sg}"
INSTANCE_NAME="dragon-scale-training"
...
launch_instance() {
    ...
    instance_id=$(aws ec2 run-instances \
        ...
        --tag-specifications "[{
            ...
            \"Tags\": [
                ...
                {\"Key\": \"Project\", \"Value\": \"DRAGON_SCALE\"},
    ...
    log ""
    log " After SSH, run:"
    log "   bash dragon-scale-core/training/ec2-setup.sh"
    log "========================================"

    echo "${instance_id}" > .dragon-scale-instance-id
    echo "${public_ip}"   > .dragon-scale-instance-ip
}
...
# ── Main ──────────────────────────────────────────────────────────────────────
main() {
    log "=== DRAGON_SCALE EC2 Provisioning ==="
    launch_instance
    print_upgrade_commands
}
