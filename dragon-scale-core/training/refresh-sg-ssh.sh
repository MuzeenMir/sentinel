...
ID_FILE=".dragon-scale-instance-id"
if [ ! -f "$ID_FILE" ]; then
  echo "  No .dragon-scale-instance-id in $(pwd). Run from dragon-scale-core/."
  exit 1
fi
...
echo ""
echo "=== Try SSH ==="
echo "  ssh -i dragon-scale-training.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=accept-new ubuntu@${PUBLIC_IP}"
echo ""
echo "If that fails:"
