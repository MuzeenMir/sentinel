# DRAGON_SCALE Training Pipeline
...
# 3. SSH into instance
ssh -i dragon-scale-training.pem ubuntu@<IP>

# 4. Set up environment (on EC2)
bash dragon-scale-core/training/ec2-setup.sh

# 5. Download datasets (on EC2)
bash dragon-scale-core/training/download_datasets.sh
...
sudo bash dragon-scale-core/training/ec2-add-swap.sh
...
```bash
INSTANCE_ID=$(cat .dragon-scale-instance-id)
...
```bash
# From local machine
scp -i dragon-scale-training.pem -r \
    ubuntu@<IP>:~/dragon-scale/dragon-scale-core/backend/ai-engine/trained_models/ \
    ./backend/ai-engine/trained_models/
```
...
aws s3 sync backend/ai-engine/trained_models/ \
    s3://your-bucket/dragon-scale-models/ \
