# DRAGON_SCALE Setup Guide

Complete setup guide for DRAGON_SCALE Security Platform.
...
# Edit dragonscaleenv
nano dragonscaleenv  # or use your preferred editor
```
...
# Or manually:
docker compose --env-file dragonscaleenv up -d
```
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file dragonscaleenv up -d
...
2. Login with credentials from `dragonscaleenv`
...
docker compose exec postgres pg_isready -U dragon-scale
```
...
docker compose --env-file dragonscaleenv up -d postgres
...
**Invalid Credentials:**
- Verify username/password in `dragonscaleenv`
...
**High Memory Usage:**
- Reduce `BATCH_SIZE` in `dragonscaleenv`
...
- **Security:** security@dragon-scale.example.com
