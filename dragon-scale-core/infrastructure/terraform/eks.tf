...
resource "aws_security_group" "eks_cluster" {
  ...
  description = "Security group for DRAGON_SCALE EKS cluster control plane"
...
resource "aws_eks_cluster" "dragon-scale" {
...
resource "aws_kms_key" "eks" {
  description             = "KMS key for DRAGON_SCALE EKS secrets encryption"
...
resource "aws_security_group" "eks_nodes" {
  ...
  description = "Security group for DRAGON_SCALE EKS worker nodes"
  vpc_id      = aws_vpc.dragon_scale_vpc.id
...
resource "aws_eks_node_group" "dragon-scale" {
  cluster_name    = aws_eks_cluster.dragon-scale.name
...
data "tls_certificate" "eks" {
  url = aws_eks_cluster.dragon-scale.identity[0].oidc[0].issuer
}
