...
resource "aws_iam_role" "host_agent" {
  name = "${var.environment}-dragon-scale-host-agent-role"
...
resource "aws_iam_role_policy" "host_agent_secrets" {
  name = "${var.environment}-dragon-scale-host-agent-secrets"
...
resource "aws_iam_instance_profile" "host_agent" {
  name = "${var.environment}-dragon-scale-host-agent-profile"
  role = aws_iam_role.host_agent.name
...
resource "aws_security_group" "host_agent" {
  name        = "${var.environment}-dragon-scale-host-agent-sg"
  description = "Security group for DRAGON_SCALE host-agent EC2 instances"
...
resource "aws_launch_template" "host_agent" {
  name_prefix   = "${var.environment}-dragon-scale-host-agent-"
  ...
  user_data = base64encode(templatefile("${path.module}/templates/host_agent_userdata.sh.tpl", {
    ...
    environment          = var.environment
    kafka_bootstrap      = aws_msk_cluster.dragon-scale.bootstrap_brokers_tls
    redis_address        = aws_elasticache_replication_group.dragon_scale_redis.primary_endpoint_address
    ecr_base_url         = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com"
    hids_image           = "${aws_ecr_repository.dragon_scale_services["hids-agent"].repository_url}:latest"
    hardening_image      = "${aws_ecr_repository.dragon_scale_services["hardening-service"].repository_url}:latest"
  ...
  tag_specifications {
    ...
    tags = {
      Name        = "${var.environment}-dragon-scale-host-agent"
      Environment = var.environment
      Project     = "DRAGON_SCALE"
      Role        = "host-agent"
    }
...
resource "aws_autoscaling_group" "host_agent" {
  ...
  tag {
    key                 = "Name"
    value               = "${var.environment}-dragon-scale-host-agent"
