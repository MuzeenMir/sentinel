#!/bin/bash
# DRAGON_SCALE Host Agent bootstrap script
# Runs on Amazon Linux 2023 at first boot.
# Installs Docker, authenticates to ECR, and starts hids-agent and hardening-service.

set -euo pipefail
exec > >(tee /var/log/dragon-scale-bootstrap.log | logger -t dragon-scale-bootstrap -s 2>/dev/console) 2>&1

echo "=== DRAGON_SCALE Host Agent Bootstrap ==="
...
docker run -d \
  --name dragon-scale-hardening-service \
  --restart unless-stopped \
  --privileged \
  -e KAFKA_BOOTSTRAP_SERVERS="${kafka_bootstrap}" \
  -e REDIS_URL="rediss://${redis_address}:6379" \
  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/dragon-scale-bootstrap.log",
            "log_group_name": "/dragon-scale/${environment}/host-agents/bootstrap",
...
echo "=== DRAGON_SCALE Host Agent Bootstrap complete ==="
