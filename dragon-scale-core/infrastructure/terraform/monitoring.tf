...
resource "aws_cloudwatch_log_group" "services" {
  ...
  name              = "/dragon-scale/${var.environment}/${each.value}"
...
resource "aws_sns_topic" "dragon_scale_alerts" {
...
resource "aws_sns_topic_subscription" "email" {
  ...
  topic_arn = aws_sns_topic.dragon_scale_alerts.arn
  protocol  = "email"
  endpoint  = var.alert_email
}

resource "aws_sns_topic_policy" "dragon_scale_alerts" {
  arn = aws_sns_topic.dragon_scale_alerts.arn
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Action   = "sns:Publish"
      Resource = aws_sns_topic.dragon_scale_alerts.arn
    }]
  })
}
...
resource "aws_cloudwatch_metric_alarm" "eks_cpu_high" {
  ...
  alarm_actions       = [aws_sns_topic.dragon_scale_alerts.arn]
  ok_actions          = [aws_sns_topic.dragon_scale_alerts.arn]

  dimensions = {
    ClusterName = aws_eks_cluster.dragon-scale.name
  }
...
resource "aws_cloudwatch_metric_alarm" "alb_5xx_high" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.dragon-scale.arn_suffix
  }
...
resource "aws_cloudwatch_metric_alarm" "redis_cpu_high" {
  ...
  dimensions = {
    ReplicationGroupId = aws_elasticache_replication_group.dragon-scale.id
  }
