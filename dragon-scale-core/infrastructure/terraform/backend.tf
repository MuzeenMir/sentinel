...
#     --table-name dragon-scale-terraform-locks \
...
terraform {
  ...
  backend "s3" {
    ...
    bucket = "dragon-scale-terraform-state-REPLACE_WITH_ACCOUNT_ID"
    key    = "dragon-scale/terraform.tfstate"
    region = "us-east-1"

    # DynamoDB table for state locking
    dynamodb_table = "dragon-scale-terraform-locks"

    # Encrypt state at rest
    encrypt = true

    # Enable server-side encryption with KMS (optional — remove if using default SSE-S3)
    # kms_key_id = "alias/dragon-scale-terraform-state"
  }
}
