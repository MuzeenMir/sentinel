# DRAGON_SCALE Python SDK

Client library for interacting with the DRAGON_SCALE Security Platform API.
...
```bash
pip install dragon-scale-sdk
```

Or install from source:

```bash
cd dragon-scale-core/sdk
pip install -e .
```
...
```python
from dragon_scale_sdk import DragonScaleClient

client = DragonScaleClient(
    base_url="https://dragon-scale.example.com",
    username="analyst1",
...
Build custom detectors that integrate with the DRAGON_SCALE ensemble.
...
```python
import numpy as np
from dragon_scale_sdk.detectors import BaseDetector
...
```python
from dragon_scale_sdk.ensemble import EnsembleManager

manager = EnsembleManager(base_url="https://dragon-scale.example.com", token="<jwt>")
...
```python
from dragon_scale_sdk.features import BaseFeatureExtractor
...
```python
from dragon_scale_sdk.exceptions import (
    DragonScaleAuthError,
    DragonScaleAPIError,
    DragonScaleRateLimitError,
    DragonScaleConnectionError,
)

try:
    result = client.detect(traffic_data)
except DragonScaleAuthError:
    # Token expired or invalid -- re-authenticate
    client.authenticate()
    result = client.detect(traffic_data)
except DragonScaleRateLimitError as e:
    # Back off and retry
    time.sleep(e.retry_after)
    result = client.detect(traffic_data)
except DragonScaleAPIError as e:
    # Server-side error
    print(f"API error {e.status_code}: {e.message}")
except DragonScaleConnectionError:
    # Network connectivity issue
    print("Cannot reach DRAGON_SCALE API")
```
...
|------------------------|------------------------------------------|
| `DRAGON_SCALE_BASE_URL`   | API base URL                             |
| `DRAGON_SCALE_USERNAME`   | Authentication username                  |
| `DRAGON_SCALE_PASSWORD`   | Authentication password                  |
| `DRAGON_SCALE_TOKEN`      | Pre-obtained JWT token                   |
| `DRAGON_SCALE_TIMEOUT`    | Request timeout in seconds (default 30)  |
| `DRAGON_SCALE_VERIFY_SSL` | SSL verification (`true`/`false`)        |
