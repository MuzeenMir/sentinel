...
```bash
git clone https://github.com/MuzeenMir/dragon-scale.git
cd dragon-scale/dragon-scale-core
```

### 2. Environment Configuration

Create `dragonscaleenv` file:

```bash
# Database
DATABASE_URL=postgresql://dragon-scale:dragon_scale_password@localhost:5432/dragon_scale_db
POSTGRES_PASSWORD=dragon_scale_password
...
```bash
docker compose --env-file dragonscaleenv up -d postgres redis zookeeper kafka
```

### 4. Start Application Services

```bash
docker compose --env-file dragonscaleenv up -d auth-service api-gateway data-collector alert-service ai-engine
```
...
```
dragon-scale-core/reverse-proxy/certs/fullchain.pem
dragon-scale-core/reverse-proxy/certs/privkey.pem
```
...
docker compose --env-file dragonscaleenv -f docker-compose.yml -f docker-compose.prod.yml up -d
```
...
for service in api-gateway auth-service alert-service data-collector ai-engine drl-engine policy-orchestrator xai-service compliance-engine; do
  docker build -t dragon-scale-$service ./backend/$service
  docker tag dragon-scale-$service:latest <account>.dkr.ecr.us-east-1.amazonaws.com/prod-dragon-scale-$service:latest
  docker push <account>.dkr.ecr.us-east-1.amazonaws.com/prod-dragon-scale-$service:latest
done
```
...
```bash
eksctl create cluster --name dragon-scale-cluster --region us-east-1 --nodegroup-name standard-workers --node-type m5.large --nodes 3
```
...
```bash
docker exec -it dragon-scale-postgres psql -U dragon-scale -d dragon_scale_db -f /docker-entrypoint-initdb.d/init.sql
```
...
aws s3 cp training_data/ s3://dragon-scale-ml-models/training/ --recursive

# Trigger SageMaker training job
aws sagemaker create-training-job --training-job-name dragon-scale-initial-training ...
```
