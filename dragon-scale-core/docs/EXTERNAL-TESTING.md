# DRAGON_SCALE External Testing Program
...
DRAGON_SCALE is a defensive platform, not a frontier LLM, so its external testing program
looks different — but the discipline transfers directly. This document names the
categories of external reviewer DRAGON_SCALE should seek out, what each is supposed to
...
1. **Adversarial robustness.** Confirm that the ensemble and the PPO agent survive
   contact with an attacker who is not on the DRAGON_SCALE team.
2. **Operator ergonomics.** Confirm that a SOC analyst who has never seen DRAGON_SCALE
...
4. **Supply chain assurance.** Confirm that DRAGON_SCALE's dependency stack, container
...
- Run a black-box pen test against a DRAGON_SCALE deployment, with knowledge of the
...
qualification under a real audit. A recommendation on whether DRAGON_SCALE is ready for a
...
- Answer, in writing: what does DRAGON_SCALE claim about itself? Where is the strongest
...
the DRAGON_SCALE team. Candidates include former frontier-AI safety reviewers, SOC
...
"helpful-only" variants, some did not, and the difference mattered. DRAGON_SCALE's analogue
...
3. **Bug bounty.** A standing bug bounty is a different kind of external testing —
   always-on, lower per-finding depth, higher volume. Should DRAGON_SCALE run one from the
...
4. **Public findings disclosure.** Mythos publishes a high-level summary of external
   findings in the system card itself. Should DRAGON_SCALE's equivalent (the
   DRAGON_SCALE-System-Card.pdf at the project root) include the same? The argument for
   is transparency; the argument against is that it gives a live attacker a reading
   list.
5. **Conflict of interest.** If the same pen-test firm reviews DRAGON_SCALE and is also a
