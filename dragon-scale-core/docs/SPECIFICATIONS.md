# DRAGON_SCALE Specification Documents

This project maintains a suite of professional software specification documents that define how DRAGON_SCALE is designed, built, tested, deployed, and maintained. These documents are the authoritative reference for all development activities.
...
|----------|----|-------------|
| **SRS.md** | DRAGON_SCALE-SRS-001 | Software Requirement Specification. Defines all functional and non-functional requirements (IEEE 830). |
| **SDD.md** | DRAGON_SCALE-SDD-001 | Software Design Document. Describes internal design of every component, data design, interfaces, and patterns (IEEE 1016). |
| **SAD.md** | DRAGON_SCALE-SAD-001 | Software Architecture Document. Architectural views (building block, runtime, deployment), ADRs, quality attributes (Arc42). |
| **STP.md** | DRAGON_SCALE-STP-001 | Software Test Plan. Test strategy, levels, environments, entry/exit criteria, acceptance criteria. |
| **SDP.md** | DRAGON_SCALE-SDP-001 | Software Development Plan. Methodology, workflow, CI/CD, roadmap, maintenance plan, extension guide. |
| **DEPLOYMENT_AND_OPERATIONS.md** | DRAGON_SCALE-DOG-001 | Deployment and Operations Guide. All deployment models, configuration, monitoring, DR, scaling, troubleshooting. |
| **SECURITY_ARCHITECTURE.md** | DRAGON_SCALE-SECARCH-001 | Security Architecture Document. Threat model, auth architecture, encryption, compliance mapping, incident response. |
| **API_SPECIFICATION.md** | DRAGON_SCALE-API-001 | API Specification. Complete endpoint catalog, event schemas, SSE streams, webhook payloads, SDK guidelines. |
...
DRAGON_SCALE now maintains a set of governance and safety documents modelled on the
discipline used by frontier-AI system cards. These define what DRAGON_SCALE claims about
...
A comparative analysis between DRAGON_SCALE and the Claude Mythos Preview System Card,
along with a first-draft DRAGON_SCALE System Card PDF, lives at the repository root:

- `DRAGON_SCALE-vs-Mythos-Comparison.md`
- `DRAGON_SCALE-System-Card.pdf`
