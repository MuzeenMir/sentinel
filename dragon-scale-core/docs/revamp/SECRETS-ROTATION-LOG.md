# DRAGON_SCALE — Secrets Rotation Log
...
|---|---|---|---|---|---|---|
| 4 | `generic-api-key` | `dragon-scale-core/dragonscaleenv` line 5–6 (`SECRET_KEY=…`) | `cb03d3c2` | 2026-01-28 | **OPEN — rotate** | @MuzeenMir |
| 5 | `generic-api-key` | `dragon-scale-core/dragonscaleenv` line 9–10 (`…PASSWORD=…`) | `cb03d3c2` | 2026-01-28 | **OPEN — rotate** | @MuzeenMir |

**What happened.** A `.env`-style file named `dragonscaleenv` was committed into
`dragon-scale-core/` in late January 2026 and later removed from the working tree.
It remains reachable in reflog and on any fork/clone that existed before the
removal. `dragonscaleenv` and `dragon_scale_env/` are now in `.gitignore` (lines
...
4. **Optional but recommended — history rewrite.** `git-filter-repo` to purge
   `dragon-scale-core/dragonscaleenv` across all refs, then force-push to the
   canonical remote (`github.com/MuzeenMir/dragon-scale`). This is destructive:
...
`dragon_scale_env/` is now gitignored (lines 19, 60 of `/.gitignore`) so no new
...
|---|---|---|---|
| 1 | `aws-access-token` | `dragon_scale_env/lib/python3.12/site-packages/PIL/ImageFont.py:1255` | Pillow source code regex collision (not a real key) |
| 2 | `generic-api-key` | `dragon_scale_env/lib/python3.12/site-packages/onnx/reference/ops/aionnxml/op_dict_vectorizer.py:42` | ONNX source code (`keys = …` literal) |
| 3 | `generic-api-key` | `dragon_scale_env/lib/python3.12/site-packages/onnxruntime/tools/symbolic_shape_infer.py:2198–2199` | ONNX Runtime source code |

Action taken: `.gitleaks.toml` allowlist extended to cover `dragon_scale_env/**`
...
- This log is tracked in git at `dragon-scale-core/docs/revamp/SECRETS-ROTATION-LOG.md`
