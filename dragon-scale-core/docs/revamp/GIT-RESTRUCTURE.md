# DRAGON_SCALE — Git Repository Restructure

| Field          | Value                                             |
|----------------|---------------------------------------------------|
| Document ID    | DRAGON_SCALE-GIT-001                                  |
| Version        | 1.0.0                                             |
...
Define how the DRAGON_SCALE repository will be structured, branched, gated, and
...
| Aspect | Current | Gap |
|---|---|---|
| Remote | `https://github.com/MuzeenMir/dragon-scale` (single `origin`) | OK |
...
| Repo shape | Monorepo: parent `dragon-scale/` wrapping `dragon-scale-core/` (the actual product) plus root-level docs | **OK but oddly nested** — see §4 for cleanup |
...
DRAGON_SCALE stays a single repository. Reasons:
...
Current shape: `dragon-scale/` → `dragon-scale-core/<everything>` plus a few root-level
files (`CLAUDE.md`, `README.md`, `LICENSE`, `DRAGON_SCALE System Card.pdf`).

The wrapping `dragon-scale/` directory adds no value — every path is one extra
segment for no payoff. Two options; pick one in Phase 0 (track as ADR-011):

- **Option A (preferred): flatten.** Move everything from `dragon-scale-core/` up
  to repo root. Move root-level wrapper files into `docs/` or `top-level/`.
  Result: `backend/`, `frontend/`, `agent/`, `docs/`, etc. directly at root.
- **Option B: rename.** Keep nesting, rename `dragon-scale-core/` to `app/` (or
...
```
# default: tech lead
*                                       @dragon-scale/tech-lead

# backend services
/backend/collector/                     @dragon-scale/backend
/backend/analyzer/                      @dragon-scale/backend @dragon-scale/ml
/backend/controller/                    @dragon-scale/backend @dragon-scale/security
/backend/console/                       @dragon-scale/backend @dragon-scale/frontend
/backend/llm-gateway/                   @dragon-scale/ai @dragon-scale/security
/backend/_lib/                          @dragon-scale/backend @dragon-scale/tech-lead

# AI / models / safety
/backend/analyzer/triage/               @dragon-scale/ai
/backend/analyzer/models/               @dragon-scale/ml
/tests/safety/                          @dragon-scale/security @dragon-scale/ai
/docs/models/                           @dragon-scale/ai
/docs/revamp/                           @dragon-scale/tech-lead

# OPA policy bundles
/opa-bundles/                           @dragon-scale/security @dragon-scale/policy

# Helm + infra
/deploy/helm/                           @dragon-scale/platform
/infrastructure/                        @dragon-scale/platform
/.github/workflows/                     @dragon-scale/platform @dragon-scale/tech-lead

# Agent
/agent/                                 @dragon-scale/endpoint @dragon-scale/security

# Frontend
/frontend/                              @dragon-scale/frontend

# Docs (revamp set is gated to tech lead; v1 docs are anyone)
/docs/specifications/                   @dragon-scale/tech-lead
```
...
- Audit `.gitignore` for entries pinning ad-hoc local files (`.dragon-scale-instance-id`,
  `.dragon-scale-instance-ip`, `dragon_scale_env`) — these should not be committed.
...
- The `DRAGON_SCALE System Card.pdf` (~117 KB) at root: acceptable in git for now;
...
- Future: `git secrets --register-aws` patterns plus regex for DRAGON_SCALE-specific
  IDs.
...
1. **PR-A: Branch protection settings.** Apply §3.4 settings via
   `gh api repos/MuzeenMir/dragon-scale/branches/main/protection` script in
...
Email security@<dragon-scale-domain> with details. Encrypt with the PGP key at
...
- **Remote location.** `https://github.com/MuzeenMir/dragon-scale` stays. No
  re-host required.
