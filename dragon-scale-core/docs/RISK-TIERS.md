# DRAGON_SCALE Product Risk Tiers
...
This document defines DRAGON_SCALE's **product risk tiers** — explicit, testable claims about
...
labs (for example, the Claude Mythos Preview System Card §2). DRAGON_SCALE is not a frontier
...
flows, the DRAGON_SCALE ensemble (XGBoost + LSTM + Isolation Forest + Autoencoder + meta-learner)
achieves:
...
- `compliance-readiness.md` — full Common Criteria → DRAGON_SCALE mapping.
- `SECURITY_ARCHITECTURE.md` (in the gitignored spec suite) — DRAGON_SCALE-SECARCH-001.
