# DRAGON_SCALE Contamination Policy
...
This document describes DRAGON_SCALE's rules for keeping training and evaluation data
...
model's training corpus and describes an auditor-based detection scheme. DRAGON_SCALE's
...
All three are realistic for DRAGON_SCALE, and the third is the one that a naive retraining
...
The `/api/v1/feedback` endpoint is DRAGON_SCALE's primary retraining signal. The feedback
loop is:
