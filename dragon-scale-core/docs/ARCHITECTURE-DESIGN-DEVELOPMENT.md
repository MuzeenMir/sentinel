# DRAGON_SCALE: Software Architecture, Design, and Development

This document is the single entry point for understanding how the DRAGON_SCALE platform is built, why it is designed the way it is, and how to develop and extend it. It is intended for developers, architects, and operations staff.
...
DRAGON_SCALE is an enterprise-grade, AI-powered security platform that provides:
...
```bash
cd dragon-scale-core/frontend/admin-console
npm install
npm run dev
```
...
```bash
cd dragon-scale-core/backend/<service>
python -m venv venv
