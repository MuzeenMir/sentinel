# DRAGON_SCALE Disaster Recovery

This document outlines backup, restore, and failover procedures for DRAGON_SCALE.
...
1. Create a backup:
```bash
pg_dump -Fc -h <db-host> -U dragon-scale -d dragon_scale_db > dragon_scale_db.dump
```
...
Back up `dragon-scale-core/backend/ai-engine/trained_models/` (or mounted volume).

### Config cache

Persisted in Redis under `dragon-scale:config`. Include Redis backup in DR.
...
```bash
pg_restore -h <db-host> -U dragon-scale -d dragon_scale_db dragon_scale_db.dump
```
