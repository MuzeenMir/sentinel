# DRAGON_SCALE Security Architecture

This document describes the security patterns, controls, and practices embedded throughout the DRAGON_SCALE platform.
...
DRAGON_SCALE uses JSON Web Tokens (JWTs) for stateless authentication across all services.
...
DRAGON_SCALE implements RBAC with five predefined roles:
...
- Each service presents a client certificate signed by the DRAGON_SCALE internal CA.
...
- All services communicate over the `dragon-scale-network` bridge network.
...
- Default deny-all `NetworkPolicy` on the DRAGON_SCALE namespace.
