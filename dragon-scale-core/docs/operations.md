# DRAGON_SCALE Operations Manual

Deployment, configuration, monitoring, and troubleshooting guide for DRAGON_SCALE platform operators.
...
```bash
kubectl create namespace dragon-scale
```

**Secrets:**

```bash
kubectl create secret generic dragon-scale-secrets \
  --namespace dragon-scale \
...
**Deployments** for: all DRAGON_SCALE services (auth, ai-engine, drl-engine, etc.).
...
kind: Ingress
metadata:
  name: dragon-scale-ingress
  namespace: dragon-scale
  annotations:
    nginx.ingress.kubernetes.io/ssl-redirect: "true"
spec:
  tls:
    - hosts: [dragon-scale.example.com]
      secretName: dragon-scale-tls
  rules:
    - host: dragon-scale.example.com
      http:
        paths:
          - path: /api
            pathType: Prefix
...
- Private subnet A: EKS worker nodes for DRAGON_SCALE services.
...
The training pipeline in `dragon-scale-core/training/spot_handler.py` supports AWS Spot interruption handling for cost-effective model training.
...
|-----------------------|------------------------|---------------------------------|
| `POSTGRES_DB`         | `dragon_scale_db`          | Database name                   |
| `POSTGRES_USER`       | `dragon-scale`             | Database user                   |
...
| `BACKUP_DIR`          | `/var/lib/dragon-scale/backups` | Hardening backup directory        |
...
Prometheus scrapes metrics from all DRAGON_SCALE services at `/metrics` endpoints (provided by the shared `metrics` module).

**Key metrics:**

| Metric                          | Type      | Description                                |
|---------------------------------|-----------|--------------------------------------------|
| `dragon_scale_threats_detected`     | Counter   | Threats detected by severity               |
| `dragon_scale_alerts_created`       | Counter   | Alerts created by severity                 |
| `dragon_scale_drl_decisions`        | Counter   | DRL decisions by action type               |
| `dragon_scale_detection_latency`    | Histogram | AI detection latency                       |
| `dragon_scale_hardening_posture`    | Gauge     | Current hardening posture score (0-100)    |
| `dragon_scale_ebpf_events`         | Counter   | eBPF events by type                        |
| `dragon_scale_fim_alerts`           | Counter   | File integrity monitoring alerts           |
...
- **DRAGON_SCALE Overview:** System-wide health, request rates, error rates.
...
0 2 * * * docker exec dragon-scale-postgres pg_dump -U dragon-scale dragon_scale_db | gzip > /backups/dragon_scale_$(date +\%Y\%m\%d).sql.gz
```

**Restore:**

```bash
gunzip < /backups/dragon_scale_20260401.sql.gz | docker exec -i dragon-scale-postgres psql -U dragon-scale dragon_scale_db
```
...
```bash
docker exec dragon-scale-redis redis-cli BGSAVE
```
...
```bash
docker run --rm -v dragon-scale-core_ai_models:/data -v /backups:/backup alpine tar czf /backup/models_$(date +%Y%m%d).tar.gz /data
```

### Hardening Backups

The hardening service automatically backs up files before remediation to `BACKUP_DIR` (`/var/lib/dragon-scale/backups`). Files are named `<sanitized_path>.<timestamp>.bak`.
...
- `dragon-scale-network-events`: 12 partitions (scale with data collector instances).
- `dragon-scale-host-events`: 6 partitions (scale with HIDS agent count).
- `extracted_features`: 6 partitions.
- `dragon-scale-hardening-events`: 3 partitions.
...
1. `docker exec dragon-scale-kafka kafka-consumer-groups --bootstrap-server localhost:9092 --describe --all-groups`.
...
2. Redis: run `docker exec dragon-scale-redis redis-cli INFO memory` to check usage.
...
1. Confirm PostgreSQL is healthy: `docker exec dragon-scale-postgres pg_isready -U dragon-scale`.
