# DRAGON_SCALE Model Red-Team Plan
...
This document is DRAGON_SCALE's equivalent of the "Frontier Red Team" section of the Claude
Mythos Preview System Card (§3.3 and §4.2–4.4). Its job is to name the adversarial tests
DRAGON_SCALE runs against its own models, say what they are trying to rule out, and track
which of them are currently wired into CI.

DRAGON_SCALE is a defensive product. But the detectors and the PPO agent inside it are
...
| Profile | Access | Goal | What DRAGON_SCALE must still do |
|---------|--------|------|-----------------------------|
| **Script-kiddie** | Public tooling, off-the-shelf exploit kits | Breach a DRAGON_SCALE-protected host via a known CVE | Detect and block via XGBoost + signature rules |
| **Evasive attacker** | Knows DRAGON_SCALE is deployed; can mutate traffic; knows the ensemble exists | Reach the host without triggering a DENY | Detect via at least one detector in the ensemble, or via HIDS post-landing |
| **Insider / red team** | Has read access to DRAGON_SCALE's model artefacts, training data, and/or architecture | Find a gradient-based adversarial example or a reward-hacking policy | Limit blast radius via reversible actions, shadow-mode gating, and auto-rollback |
...
- A sophisticated state actor who can compromise the DRAGON_SCALE control plane itself. That
...
4. Do we need a human red team in addition to these automated suites? Frontier labs use
   both. For DRAGON_SCALE, the highest-value human red-team activity is probably a
