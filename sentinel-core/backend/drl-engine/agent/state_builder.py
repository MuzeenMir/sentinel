...
_FEATURE_META: List[Dict[str, str]] = [
    ...
    {"name": "confidence", "description": "Model confidence score [0-1]"},
    {
        ...
        "description": "Target asset criticality normalised to [0-1]",
    },
    {"name": "traffic_volume", "description": "Packet/byte volume clipped at 50 000"},
    {
        "name": "protocol_risk",
        "description": "L4 protocol risk (TCP=0.3, UDP=0.4, ICMP=0.5)",
    },
    {"name": "time_risk", "description": "Time-of-day risk [0-1]"},
    {
        "name": "historical_alerts",
        "description": "Prior alert count for this source, clipped at 500",
    },
    {
        "name": "is_internal",
        "description": "1.0 if source is RFC-1918 private, else 0.0",
    },
    {
        "name": "port_sensitivity",
        "description": "Destination port risk classification [0-1]",
    },
    ...
    def build_state(self, data: Dict[str, Any]) -> np.ndarray:
        ...
        vec = np.array(
            [
                ...
                self._normalize(
                    float(data.get("asset_criticality", 1)), 1.0, 5.0, clip=True
                ),
                # 3 — traffic_volume (linear clip at 50 000)
                min(float(data.get("traffic_volume", 0)) / 50_000.0, 1.0),
                # 4 — protocol_risk
                _PROTOCOL_RISK.get(
                    str(data.get("protocol", "")).upper(), _PROTOCOL_DEFAULT
                ),
                ...
                min(
                    float(
                        data.get(
                            "historical_alert_count", ctx.get("historical_alerts", 0)
                        )
                    )
                    / 500.0,
                    1.0,
                ...
                self._log_norm(
                    float(ctx.get("connection_rate", data.get("connection_rate", 0))),
                    scale=1000.0,
                ),
                # 11 — geo_risk
