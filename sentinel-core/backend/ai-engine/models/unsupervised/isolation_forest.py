...
class IsolationForestDetector(BaseDetector):
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            scaler_file = os.path.join(
                self.model_path, "isolation_forest_scaler.joblib"
            )
    ...
    def calibrate_threshold(
        self, X_benign: np.ndarray, X_attack: np.ndarray, target_fpr: float = 0.05
    ) -> float:
        """
        ...
        actual_fpr = np.sum(benign_scores < self._threshold) / max(
            len(benign_scores), 1
        )
