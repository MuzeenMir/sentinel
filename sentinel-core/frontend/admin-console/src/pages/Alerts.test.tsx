...
describe("Alerts", () => {
  beforeEach(() => {
    ...
    mockGetAlerts.mockResolvedValue({
      data: { alerts: sampleAlerts, total: 3 },
    });
    mockGetStats.mockResolvedValue({
      data: { new: 1, acknowledged: 1, resolved: 1, ignored: 0 },
    });
  ...
  it("shows error state when API fails", async () => {
    ...
    expect(
      await screen.findByText(/failed to load alerts/i),
    ).toBeInTheDocument();
  });
  ...
  it("does not show action buttons for resolved alerts", async () => {
    ...
    const bruteForceRow = rows.find((row) =>
      within(row).queryByText("Brute Force"),
    );
