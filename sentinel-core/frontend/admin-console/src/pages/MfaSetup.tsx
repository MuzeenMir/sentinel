...
import { ShieldCheck, QrCode, Key, AlertTriangle, Check, X } from 'lucide-react'
    ...
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-semibold text-white mt-1">
              {mfaEnabled ? 'Enabled (TOTP)' : enrolled ? 'Enrolled (not verified)' : 'Not configured'}
            </p>
            {statusData?.has_backup_codes && (
              <p className="text-xs text-green-400 mt-1">Backup codes generated</p>
            )}
          </div>
      ...
      {(step === 'status' || step === 'enroll') && !mfaEnabled && (
        <div className="card p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Step 1 — Enroll</h2>
          </div>
          <p className="text-sm text-slate-400">
            Click below to generate a TOTP secret. You'll then scan the QR code with your
      ...
      {step === 'verify' && (
        <div className="card p-6 space-y-4">
          <div className="flex items-center gap-3">
            <QrCode className="h-5 w-5 text-cyan-400" />
            <h2 className="text-lg font-semibold text-white">Step 2 — Scan &amp; Verify</h2>
          </div>
          <p className="text-sm text-slate-400">
          ...
          <p className="text-xs text-slate-500 break-all font-mono">{provisioningUri}</p>
      ...
      {step === 'backup' && (
        <div className="card p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Step 3 — Backup Codes</h2>
          </div>
          <p className="text-sm text-slate-400">
            MFA is now enabled. Generate backup codes in case you lose access to your authenticator.
          ...
          {backupCodes.length > 0 ? (
            <>
              <div className="grid grid-cols-2 gap-2">
                {backupCodes.map((c) => (
                  <code key={c} className="rounded bg-slate-800 px-3 py-1.5 text-sm font-mono text-cyan-300">
                    {c}
                  </code>
                ))}
              </div>
            ...
            >
              ...
              {backupMutation.isPending ? 'Generating…' : 'Generate Backup Codes'}
            </button>
          )}
        </div>
      )}
      ...
      {mfaEnabled && step !== 'disable' && (
        <div className="card p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              onClick={() => { backupMutation.mutate(); setStep('backup') }}
              className="btn-secondary gap-2"
            >
      ...
      {step === 'disable' && (
        <div className="card p-6 space-y-4 border border-red-500/20">
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-sm text-slate-400 mb-1">Current TOTP code</label>
              <input
                type="text"
                maxLength={6}
                value={disableCode}
