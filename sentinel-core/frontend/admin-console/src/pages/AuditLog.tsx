...
import { ClipboardList, Filter, BarChart2, ShieldCheck, ShieldAlert } from 'lucide-react'
...
function categoryBadge(category: string) {
  const cls = CATEGORY_COLORS[category] ?? 'bg-slate-600/30 text-slate-300 border-slate-600/50'
...
export function AuditLog() {
    ...
    queryFn: () =>
      auditApi
        .getEvents({
          ...(category ? { category } : {}),
          ...(actor ? { actor } : {}),
          ...(startDate ? { start_time: new Date(startDate).getTime() / 1000 } : {}),
          ...(endDate ? { end_time: new Date(endDate + 'T23:59:59').getTime() / 1000 } : {}),
          limit: PAGE_SIZE,
          offset,
        })
        .then((r) => r.data),
  ...
  const events: AuditEntry[] = eventsQuery.data?.events ?? eventsQuery.data ?? []
  const categories: string[] = categoriesQuery.data?.categories ?? []

  const handleVerifyPage = async () => {
    if (events.length === 0) return
    try {
      const res = await auditApi.verifyIntegrity(events as unknown as Record<string, unknown>[])
          ...
          {statsQuery.isLoading ? (
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-white">Audit Statistics</h2>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                {Object.entries(statsQuery.data.by_category).map(([cat, count]) => (
      ...
      {verifyMap && (
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-white">Integrity Verification</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {verifyResult!.map((r) => (
              ...
              >
                {r.valid ? <ShieldCheck className="h-3 w-3" /> : <ShieldAlert className="h-3 w-3" />}
                {r.id.slice(0, 16)}…
              </span>
            ))}
          </div>
      ...
      <div className="card p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          {(category || actor || startDate || endDate) && (
            <button
              onClick={() => { setCategory(''); setActor(''); setStartDate(''); setEndDate(''); setPage(0) }}
                  ...
                  return (
                    <tr key={event.id} className="hover:bg-slate-800/30">
                      ...
                      <td className="table-cell">
                        {categoryBadge((event as unknown as { category: string }).category ?? event.event_type ?? '')}
                      </td>
                      <td className="table-cell">
                      ...
                      <td className="table-cell font-medium text-white text-sm">{event.user ?? (event as unknown as { actor: string }).actor}</td>
                      <td className="table-cell text-slate-300 text-sm">
                        {event.resource_type ?? (event as unknown as { resource: string }).resource}
                      </td>
                      <td className="table-cell text-slate-400 text-xs">
                        {(event as unknown as { tenant_id?: number | null }).tenant_id ?? '—'}
                      </td>
                      <td className="table-cell">
                        {valid === undefined ? (
