...
def compute_statistical_features(records: List[Dict[str, Any]]) -> StatisticalFeatures:
    ...
    if sizes:
        ...
        if len(sizes) > 1:
            variance = sum((s - features.packet_size_mean) ** 2 for s in sizes) / len(
                sizes
            )
        ...
        features.packet_size_q75 = (
            sorted_sizes[3 * n // 4] if n >= 4 else sorted_sizes[-1]
        )
    ...
    if len(timestamps) > 1:
        ...
        if iats:
            ...
            if len(iats) > 1:
                variance = sum((iat - features.iat_mean) ** 2 for iat in iats) / len(
                    iats
                )
...
def compute_flow_features(
    ...
    flow_features = {
        ...
        **behavioral.to_dict(),
        ...
        # Direction analysis
        "inbound_bytes": sum(
            r.get("bytes", 0) for r in records if r.get("direction") == "inbound"
        ),
        "outbound_bytes": sum(
            r.get("bytes", 0) for r in records if r.get("direction") == "outbound"
        ),
        "internal_bytes": sum(
            r.get("bytes", 0) for r in records if r.get("direction") == "internal"
        ),
    }
...
def aggregate_window_features(feature_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    byte_rates = [
        f.get("byte_rate", 0) for f in feature_list if f.get("byte_rate", 0) > 0
    ]
    if byte_rates:
        aggregated["window_avg_byte_rate"] = sum(byte_rates) / len(byte_rates)

    packet_rates = [
        f.get("packet_rate", 0) for f in feature_list if f.get("packet_rate", 0) > 0
    ]
    if packet_rates:
    ...
    # Aggregate entropy
    entropies = [
        f.get("src_ip_entropy", 0) + f.get("dst_ip_entropy", 0) for f in feature_list
    ]
    if entropies:
